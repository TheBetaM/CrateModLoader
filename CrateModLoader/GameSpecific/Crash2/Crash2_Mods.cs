...
namespace CrateModLoader.GameSpecific.Crash2
{
    public static class Crash2_Mods
    {
        ...
        public static void Mod_RandomWoodCrates(NSF nsf, NSD nsd, Random rand)
        {
            // edit NSF
            ...
            CrashTri_Common.Fix_Detonator(nsf, nsd);
            CrashTri_Common.Fix_BoxCount(nsf, nsd);

        }

        public static void Mod_TurnCratesIntoWumpa(NSF nsf, NSD nsd, Random rand)
        {
            // edit NSF
            foreach (Chunk chunk in nsf.Chunks)
            {
