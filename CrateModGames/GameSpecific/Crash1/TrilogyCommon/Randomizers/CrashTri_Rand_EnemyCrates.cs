...
namespace CrateModLoader.GameSpecific.Crash1.TrilogyCommon
{
    ...
    public class CrashTri_Rand_EnemyCrates : ModStruct<NSF_Pair>
    {
        ...
        private List<EntityTypePair> EnemiesToCrate_C2 = new List<EntityTypePair>()
        {
            // Turtle Woods
        ...
        private static List<EntityTypePair> EnemiesToCrate_C3 = new List<EntityTypePair>()
        {
            // Toad Village
            ...
            */
            ...
            foreach (ZoneEntry zone in pair.nsf.GetEntries<ZoneEntry>())
            {
                foreach (Entity ent in zone.Entities)
                {
                    ...
                    {
                        foreach (EntityTypePair epair in EnemiesToCrate_C2)
                        {
                            if (epair.Type == ent.Type && epair.Subtype == ent.Subtype)
                            {
            ...
            foreach (NewZoneEntry zone in pair.nsf.GetEntries<NewZoneEntry>())
            {
                if (zone.EName != "77_gZ")
                {
                    foreach (Entity ent in zone.Entities)
                    {
                        ...
                        if (ent.Type != null && ent.Subtype != null)
                        {
                            foreach (EntityTypePair epair in EnemiesToCrate_C3)
                            {
                                if (epair.Type == ent.Type && epair.Subtype == ent.Subtype)
                                {
                                    if (!isRandom || (isRandom && rand.Next(2) == 0))
